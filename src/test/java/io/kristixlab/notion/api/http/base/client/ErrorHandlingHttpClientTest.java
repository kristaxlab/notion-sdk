...
class ErrorHandlingHttpClientTest {
  ...
  void nullErrorHandlerThrows() {
    var fake = new HttpClientMock(responseWithStatus(200));
    assertThrows(NullPointerException.class, () -> new ErrorHandlingHttpClient(fake, null));
  }

  @Test
  @DisplayName("QUITE handler passes through all responses, including errors")
  void noopHandlerPassesThrough() throws IOException {
    var fake = new HttpClientMock(responseWithStatus(500, "Internal Server Error"));
  ...
  void successfulResponsePassesThrough() throws IOException {
    var fake = new HttpClientMock(responseWithStatus(200, "{\"ok\": true}"));
  ...
  void clientErrorThrows() {
    var fake = new HttpClientMock(responseWithStatus(400, "{\"code\": \"validation_error\"}"));
    ErrorResponseHandler handler =
        (req, res) -> {
          if (res.statusCode() >= 400) {
            throw new TestApiException(res.statusCode(), res.bodyAsString());
  ...
  void serverErrorThrows() {
    var fake =
        new HttpClientMock(responseWithStatus(503, "{\"message\": \"service unavailable\"}"));
    ErrorResponseHandler handler =
        (req, res) -> {
          if (res.statusCode() >= 400) {
            throw new TestApiException(res.statusCode(), res.bodyAsString());
  ...
  void composesWithInterceptingClient() {
    var fake = new HttpClientMock(responseWithStatus(401, "{\"code\": \"unauthorized\"}"));
  ...
  @Test
  @DisplayName("Exception from delegate propagates without calling error handler")
  void ioExceptionPropagates() {
    ...
    assertTrue(
        handlerLog.isEmpty(),
        "Error handler should not run when Exception is thrown in a delegate");
  }
}
