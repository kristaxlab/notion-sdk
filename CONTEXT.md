...
`Page.getProperties()`, modelled by `PageProperty`. Paginated properties may be truncated here.
_Avoid_: page property (ambiguous with the `PageProperty` class, which also models a retrieved
property value), inline property.

**Retrieved property value**:
The single-object response of the property retrieve endpoint, returned for every non-paginated
property. Nearly identical to the embedded property value and modelled by the same `PageProperty`
subclasses.
_Avoid_: property item (Notion's own word, but it collides with the SDK's `PropertyItem` class),
single-object shape.

**Retrieved property list**:
...
_Avoid_: property item list, list shape, page property list (reserve that spelling for the class
name).

**Property item metadata**:
The nested `property_item` object of a retrieved property list, modelled by `PropertyItem`. It
...
**Listed item**:
One element of the `results` array of a retrieved property list, modelled by `ListedItem` subclasses
